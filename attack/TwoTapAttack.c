...
#define LFSR1_GATE   0b001101001  // 1 + x^3 + x^4 + x^6 + x^9
#define LFSR2_GATE_A  0b101101101  // 1 + x^1 + x^3 + x^4 + x^6 + x^7 + x^9
#define LFSR2_GATE_B  0b000100001  // 1 + x^4 x^9

#define NUM_OUTPUT_BITS 64

#define KNOWN_OUTPUT_BITS 0b0101111010001110000111010001110100101110110110110111111101111001
...
int main() {
  ...
  for(int LFSR1_INITIAL_STATE = 0b000000001; LFSR1_INITIAL_STATE < 0b111111111; LFSR1_INITIAL_STATE++) {
    for(int LFSR2_INITIAL_STATE = 0b000000001; LFSR2_INITIAL_STATE < 0b111111111; LFSR2_INITIAL_STATE++) {
      outputBits = 0;
      ...
      // int LFSR1_INITIAL_STATE = 0b000000001; 
      // int LFSR2_INITIAL_STATE = 0b000000001;
      int LFSR3_INITIAL_STATE = 0b000000001;
      int lfsr3_gate  = 0b000000001;
  ...
  printf("Brute force failed! Damn ☹️.\n");
...
void clockCycle(int *lfsr1_state_ptr, int *lfsr2_state_ptr){
  ...
  //controller/LFSR1
  int lfsr1Bit = getFeedback(lfsr1_state, LFSR1_GATE, 9);
  ...
  outputBits = (outputBits << 1) | outputBit;
